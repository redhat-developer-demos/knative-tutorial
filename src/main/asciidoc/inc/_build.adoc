...
<4> Set from `image` parameter --  the fully qualified container image that will be pushed to the registry --
...
===== build 
include::_knative-objects.adoc[leveloffset=+1,tag=knative-build]

[#build-watching-logs]
=== Watching logs

The Knative build creates one init container for each build step, the init container will be named like `build-step-<step-name>`.

For the build we created will have 2 containers namely:

* build-step-git-source
* build-step-docker-push

The following command can be used to watch the log of a container within a pod,
...
To watch the logs of build pod called `docker-build-pod-2a271e` and the build step `docker-push`,
...
[source,bash]
----
kubectl logs -n knativetutorial docker-build-pod-2a271e -c build-step-build-maven
----

[TIP]
====
* You can use the following command to list names of all the init containers of a pod .
...
include::_invokeservice[tag=env]
...
[source,bash]
----
http POST $IP_ADDRESS 'Host: event-greeter.knativetutorial.example.com'  \
