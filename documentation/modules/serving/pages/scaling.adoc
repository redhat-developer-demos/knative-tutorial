...
After you've successfully run this small load test, you will notice the number of greeter service pods will have scaled to 7 automatically. 
...
[.console-output]
----
totalPodsToScale = concurrencyTarget * utilization per pod
----

By default the Knative Service utlization is set to `0.7`, hence
with this current setting of **concurrencyTarget=10** and **utilization=0.7** , you will see Knative automatically scales the greeter services to  `**10 x 0.7 = 7 pods**`.
...
[.console-input]
----
kn service delete -n {tutorial-namespace} greeter
