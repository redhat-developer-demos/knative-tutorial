...
By labeling `{tutorial-namespace}` namespace with `eventing.knative.dev/injection=enabled` as shown below, will make Knative Eventing to deploy a default Knative Eventing Broker and its related Ingress :
